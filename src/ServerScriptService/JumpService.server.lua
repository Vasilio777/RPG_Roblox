...
local JUMP_COIN_COST = 2

local function updateJumpPower(player, updateFunction)
	-- Update the jump power table
	local newJumpPower = PlayerData.updateValue(player, PlayerData.JUMP_KEY_NAME, updateFunction)
	...
	if humanoid then
		...
		Leaderboard.setStat(player, PlayerData.JUMP_KEY_NAME, newJumpPower)
	end
end

local function onPurchaseJumpIncrease(player)
	local coinAmount = PlayerData.getValue(player, PlayerData.COIN_KEY_NAME)
	...
	local newCoinAmount = PlayerData.updateValue(player, PlayerData.COIN_KEY_NAME, function(oldCoinAmount)
